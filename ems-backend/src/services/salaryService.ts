...
import {
  ...
  SalaryResponseDTO,
} from "../dTOs/response/salaryResponseDTO.js";
...
export class SalaryService {
  async getSalaries(dto: GetSalaryRequestDTO) {
    const {
      id,
      role,
      currentPage,
    ...
    const { data, totalRecords } = await salaryRepository.getAll(
      employeeId,
      role,
      pageNumber,
      pageSize,
  ...
  async getSalaryPDF(id: string): Promise<SalaryDTO | null> {
  ...
  async getSalaryDetail(id: string) {
    const employee = await employeeRepository.findByUserId(id);
    if (!employee) {
      throw new Error("Employee not found");
    }
    const salaryDetail = await salaryRepository.findSalaryDetail(
  ...
  async add(dto: AddSalaryRequestDTO) {
    ...
    const workingDays = moment(prevMonthStart).businessDiff(
      moment(prevMonthEnd)
    );

    const leaves = await leaveRepository.findApprovedLeaves(
      employeeId,
      prevMonthStart.toDate(),
      prevMonthEnd.toDate()
    );
    ...
    const netSalary = parseFloat(
      (basicSalary + allowances - deductions - leaveDeduction).toFixed(2)
    );
    ...
    await employeeRepository.updateSalary(
      employeeId,
  ...
  async processSalaries() {
    ...
    for (const employee of employees) {
      const latestSalary = await salaryRepository.findLatestSalary(
        employee._id.toString(),
        currentYear
      );
      ...
      if (!latestSalary) {
        console.log(
          `Skipping Employee ID ${employee._id}: No previous salary found.`
        );
        continue;
      }

      const { basicSalary, allowances, deductions, carryForwardLeaves } =
        latestSalary;
      const workingDays = moment(prevMonthStart).businessDiff(
        moment(prevMonthEnd)
      );
      ...
      const leaveDeduction = parseFloat(
        (excessLeaves * perDaySalary).toFixed(2)
      );
      const netSalary = parseFloat(
        (basicSalary + allowances - deductions - leaveDeduction).toFixed(2)
      );

      const newCarryForwardLeaves =
